    ...
    <header className="no-print sticky top-0 z-50 border-b border-hairline bg-background/70 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        <Link to="/" className="group flex items-center gap-2 font-display text-2xl leading-none tracking-tight">
          <span className="size-2 rounded-full bg-accent transition-transform group-hover:scale-125" aria-hidden />
          <span>Ubaldo</span>
          <span className="text-muted-foreground italic">Santos</span>
        </Link>
        ...
        <div className="flex items-center gap-1">
          <div className="hidden items-center gap-0.5 rounded-full border border-hairline bg-surface p-0.5 text-xs font-mono uppercase sm:flex" role="group" aria-label={t("actions.language")}>
            ...
            <div className="mt-3 flex items-center gap-2 px-3 pb-2">
              ...
              {SUPPORTED_LANGS.map((lng) => (
                <button
                  ...
                  onClick={() => i18n.changeLanguage(lng)}
                  className={cn(
                    "rounded-full px-2.5 py-1 text-xs font-mono uppercase",
