...
process.stdin.on("end", () => {
  ...
  if (!result || (result.error && result.error.code === "ENOENT")) {
    process.stderr.write(
      "[InsAIts] python3/python not found. Install Python 3.9+ and: pip install insa-its\n",
    );
    process.stdout.write(raw);
    process.exit(0);
  }
  ...
  if (!Number.isInteger(result.status)) {
    ...
    process.stderr.write(
      `[InsAIts] Security monitor killed (signal: ${signal}). Tool execution continues.\n`,
    );
    process.stdout.write(raw);
    process.exit(0);
  }
  ...
  if (result.status !== 0 && result.status !== 2) {
    ...
    process.stderr.write(
      `[InsAIts] Security monitor exited with status ${result.status}${suffix}\n`,
    );
    process.stdout.write(raw);
    process.exit(0);
  }
