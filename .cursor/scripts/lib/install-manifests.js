...
const LEGACY_COMPAT_BASE_MODULE_IDS_BY_TARGET = Object.freeze({
  ...
  hermes: ["rules-core", "agents-core", "commands-core", "platform-configs", "workflow-quality"],
  openclaw: ["rules-core", "agents-core", "commands-core", "platform-configs", "workflow-quality"],
  kimi: ["rules-core", "agents-core", "commands-core", "platform-configs", "workflow-quality"],
});
...
function readModuleTargetsOrThrow(module) {
  ...
  const unsupportedTargets = normalizedTargets.filter(
    (target) => !SUPPORTED_INSTALL_TARGETS.includes(target),
  );
...
function assertKnownModuleIds(moduleIds, manifests) {
  const unknownModuleIds = dedupeStrings(moduleIds).filter(
    (moduleId) => !manifests.modulesById.has(moduleId),
  );
...
function loadInstallManifests(options = {}) {
  ...
  const profiles =
    profilesData && typeof profilesData.profiles === "object" ? profilesData.profiles : {};
  const components = Array.isArray(componentsData.components)
    ? componentsData.components.slice()
    : [];
...
function resolveLegacyCompatibilitySelection(options = {}) {
  ...
  const legacyLanguages = dedupeStrings(options.legacyLanguages).map((language) =>
  ...
  const baseModuleIds =
    LEGACY_COMPAT_BASE_MODULE_IDS_BY_TARGET[target || "claude"] ||
    LEGACY_COMPAT_BASE_MODULE_IDS_BY_TARGET.claude;
  const moduleIds = dedupeStrings([
    ...baseModuleIds,
    ...(target === "antigravity"
      ? []
      : canonicalLegacyLanguages.flatMap(
          (language) => LEGACY_LANGUAGE_EXTRA_MODULE_IDS[language] || [],
        )),
  ]);
...
function resolveInstallPlan(options = {}) {
  ...
  const shouldUseTargetDefaultProfile =
    !requestedProfileId && explicitModuleIds.length === 0 && includedComponentIds.length === 0;
  ...
  return {
    ...
    warnings: targetDefaultExclusions.map(
      (exclusion) =>
