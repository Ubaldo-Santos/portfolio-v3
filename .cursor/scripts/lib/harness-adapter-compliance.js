...
const COMPLIANCE_STATES = Object.freeze({
  ...
  "Adapter-backed":
    "ECC has a thin adapter, plugin, or package surface, but parity differs by harness.",
  "Instruction-backed":
    "ECC can provide the guidance and files, but the harness does not expose the runtime hook/session surface ECC needs for enforcement.",
  "Reference-only":
    "The tool is useful as a design pressure or external runtime, but ECC does not yet ship a direct installer or adapter for it.",
});
...
const ADAPTER_RECORDS = Object.freeze(
  [
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "Native hook enforcement and Claude slash-command semantics are not equivalent",
      ],
      install_or_onramp: [
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "Event names, plugin packaging, and command dispatch differ from Claude Code",
      ],
    ...
    },
    {
      ...
      harness: "Cursor",
      state: "Adapter-backed",
      supported_assets: ["Cursor rules", "project-local skills", "hook adapter", "shared scripts"],
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "No full ECC hook parity; ecosystem ports must document drift from upstream ECC",
      ],
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "Zed external agents and native Agent Panel permissions are not Claude hooks",
      ],
      ...
      risk_notes: [
        "Keep project settings conservative and do not copy BYOK/OpenRouter secrets into `.zed/`.",
      ],
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "dmux is an orchestration runtime, not an install target for skills/rules",
      ],
      install_or_onramp: [
        "`node scripts/session-inspect.js --list-adapters`",
    ...
    },
    {
      ...
      unsupported_surfaces: [
        "No external UI, no automatic session control unless scripts are run explicitly",
      ],
      install_or_onramp: [
        "Clone repo",
...
function validateDocumentation(options = {}) {
  ...
  const docPath =
    options.docPath || path.join(repoRoot, "docs", "architecture", "harness-adapter-compliance.md");
  const errors = [];
  ...
  if (actual === null) {
    ...
    errors.push(
      `matrix block in ${path.relative(repoRoot, docPath)} is not generated from adapter records`,
    );
  }
