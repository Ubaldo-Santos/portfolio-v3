...
const PENDING_AMENDMENT_STATUSES = Object.freeze(
  new Set(["pending", "proposed", "queued", "open"]),
);
...
function collectSkillHealth(options = {}) {
  ...
  const warnThreshold =
    typeof options.warnThreshold === "number"
  ...
  const skills = Array.from(skillsById.values())
    ...
    .map((skill) => {
      ...
      const currentVersionNumber = skill.skill_dir
        ? versioning.getCurrentVersion(skill.skill_dir)
        : 0;
