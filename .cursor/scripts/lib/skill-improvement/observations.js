...
function createSkillObservation(input) {
  ...
  const skillPath =
    typeof input.skill.path === "string" && input.skill.path.trim().length > 0
      ? input.skill.path.trim()
      : null;
  const success = Boolean(input.success);
  const error = input.error === null || input.error === undefined ? null : String(input.error);
  const feedback =
    input.feedback === null || input.feedback === undefined ? null : String(input.feedback);
  const variant =
    typeof input.variant === "string" && input.variant.trim().length > 0
      ? input.variant.trim()
      : "baseline";
...
function readSkillObservations(options = {}) {
  const observationPath = path.resolve(
    options.observationsPath || getSkillObservationsPath(options),
  );
