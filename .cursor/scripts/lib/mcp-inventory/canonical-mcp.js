...
const SECRET_FLAG_PATTERN =
  /(^|[-_])(api[-_]?key|apikey|token|secret|password|passwd|auth|credential|access[-_]?key|private[-_]?key)$/i;
...
function redactArgs(args) {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    if (
      inlineMatch &&
      (SECRET_FLAG_PATTERN.test(inlineMatch[1].replace(/^--?/, "")) ||
        looksLikeSecretValue(inlineMatch[2]))
    ) {
...
function redactUrl(url) {
  ...
  safe = safe.replace(
    /([?&](?:token|key|api[_-]?key|access[_-]?token|secret)=)[^&]+/gi,
    `$1${REDACTED}`,
  );
  return safe;
}
...
function normalizeServerEntry(rawServer) {
  ...
  const argsCarrySecret =
    rawArgs.length !== args.length || rawArgs.some((value, index) => value !== args[index]);
