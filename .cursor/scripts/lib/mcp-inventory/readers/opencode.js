...
function mapOpencodeServer(name, raw, configPath) {
  ...
  const commandArray = Array.isArray(raw.command)
    ? raw.command.filter((item) => typeof item === "string")
    : [];
...
function readOpencodeMcp(options = {}) {
  ...
  const configPath = candidatePaths.find(
  ...
  const block =
    parsed && typeof parsed.mcp === "object" && parsed.mcp
      ? parsed.mcp
