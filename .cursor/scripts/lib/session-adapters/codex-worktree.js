...
function resolveSessionsDir(options = {}, context = {}) {
  const explicit =
    options.sessionsDir || context.codexSessionsDir || process.env.CODEX_SESSIONS_DIR;
...
function findRolloutById(sessionsDir, sessionId) {
  return (
    listRolloutFiles(sessionsDir).find((filePath) => path.basename(filePath).includes(sessionId)) ||
    null
  );
}
...
function resolveRolloutPath(target, cwd, options, context) {
  ...
  if (explicitTarget) {
    ...
    if (fs.existsSync(absoluteExplicit) && isRolloutFile(absoluteExplicit)) {
      return {
        rolloutPath: absoluteExplicit,
        sourceTarget: { type: "codex-rollout-file", value: absoluteExplicit },
      };
    }
  ...
  if (isCodexRolloutFileTarget(target, cwd)) {
    const absoluteTarget = path.resolve(cwd, target);
    return {
      rolloutPath: absoluteTarget,
      sourceTarget: { type: "codex-rollout-file", value: absoluteTarget },
    };
  }
...
function parseCodexRollout(rolloutPath, options = {}) {
  ...
  const isRecent =
    lastActivityMs !== null && Date.now() - lastActivityMs <= RECENT_ACTIVITY_THRESHOLD_MS;
