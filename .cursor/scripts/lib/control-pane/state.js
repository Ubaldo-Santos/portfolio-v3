...
const AGENT_OWNER_RE =
  /(agent|claude|codex|hermes|gemini|opencode|qwen|joycode|codebuddy|\bbot\b|gpt|sonnet|opus|haiku|fable)/i;
...
function classifyAssignee({ owner, sessionId, metadata = {} }) {
  ...
  if (explicitKind === "agent" || explicitKind === "human") {
    return {
      assigneeKind: explicitKind,
      assignee: owner || sessionId || metadata.assignee || null,
    };
  }
...
async function buildControlPaneSnapshot(options = {}) {
  ...
  try {
    ...
    if (options.includeProximity) {
      ...
      proximity = buildProximitySnapshot(sessions, {
        repoRoot,
