...
const DISCUSSION_ENABLED_QUERY =
...
function discussionNeedsMaintainerTouch(discussion) {
  ...
  if (discussion.answer && MAINTAINER_ASSOCIATIONS.has(discussion.answer.authorAssociation)) {
    return false;
  }

  const comments =
    discussion.comments && Array.isArray(discussion.comments.nodes)
...
function discussionNeedsAcceptedAnswer(discussion) {
  return Boolean(
    discussion && discussion.category && discussion.category.isAnswerable && !discussion.answer,
  );
}
