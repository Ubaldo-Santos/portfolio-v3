...
function createFallbackValidator() {
  const validate = (state) => {
    ...
    if (!target || typeof target !== "object" || Array.isArray(target)) {
      ...
      validateNoAdditionalProperties(target, "/target", [
        "id",
        "target",
        "kind",
        "root",
    ...
    if (!resolution || typeof resolution !== "object" || Array.isArray(resolution)) {
      ...
      validateNoAdditionalProperties(resolution, "/resolution", [
        "selectedModules",
        "skippedModules",
      ]);
    ...
    if (!source || typeof source !== "object" || Array.isArray(source)) {
      ...
      validateNoAdditionalProperties(source, "/source", [
        "repoVersion",
        "repoCommit",
        "manifestVersion",
      ]);
...
function assertValidInstallState(state, label) {
  ...
  if (!result.valid) {
    throw new Error(
      `Invalid install-state${label ? ` (${label})` : ""}: ${formatValidationErrors(result.errors)}`,
    );
  }
}
